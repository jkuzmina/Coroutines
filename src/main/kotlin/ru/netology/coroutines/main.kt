            ...
            try {
                val posts = getPosts(client)
                    .map { post ->
                        async {
                            //PostWithComments(post, getComments(client, post.id))
                            //посты с автором и комментариями
                            PostWithAuthorAndComments(PostWithComments(post, getComments(client, post.id)),  getAuthorById(client, post.authorId))
                            //посты с автором и комментарии с авторами
                            PostAndCommentsWithAuthor(post, getAuthorById(client, post.authorId), getComments(client, post.id)
                                .map{
                                comment ->
                                async { CommentWithAuthor(comment, getAuthorById(client, comment.authorId)) }
                            }.awaitAll())
                            //getComments(client, post.id).map { getAuthorById(client, it.authorId) }
